...
export default function Login() {
  ...
  React.useEffect(() => {
    ...
    if (params.get('auth') === 'success') {
      const processAuth = async () => {
        ...
        if (dataStr) {
          try {
            const data = JSON.parse(decodeURIComponent(escape(atob(decodeURIComponent(dataStr)))));
            localStorage.setItem('user', JSON.stringify(data));
            navigate('/catalogo');
          } catch {
            setError('Error al decodificar los datos de sesión');
            setLoading(false);
          }
        } else {
          setError('No se recibieron datos de sesión');
          setLoading(false);
        }
      };
      processAuth();
  ...
  return (
    <div className="bg-[#f8fafc] text-[#0f172a] min-h-screen flex items-center justify-center p-6 antialiased">
      <div className="min-card w-full max-w-[400px] p-8 flex flex-col gap-6">
        <div className="flex flex-col items-center gap-2 text-center">
          <div className="w-12 h-12 rounded-xl bg-[#4f46e5]/10 flex items-center justify-center mb-2">
            <span className="material-symbols-outlined text-[#4f46e5] text-2xl">devices</span>
          </div>
        ...
        <div className="text-center mt-2">
          <span className="text-sm text-[#64748b]">¿No tienes una cuenta?</span>
          <Link to="/registrar" className="text-sm font-medium text-[#4f46e5] hover:text-[#4338ca] ml-2 transition-colors">Regístrate</Link>
        </div>
      </div>
    </div>
  );
